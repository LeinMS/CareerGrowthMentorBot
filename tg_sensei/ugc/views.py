...
def send_checkin_messages():
    ...
    for profile in profiles:
        try:
            ...
            if current_hour_in_user_timezone == "ЭТО КОСТЫЛЬ, ЕСЛИ НУЖНО ЧТОБЫ РАБОТАЛИ УВЕДОМЛЕНИЯ, ТО ПОСТАВЬТЕ int(12)":
                try:
...
scheduler.add_job(send_checkin_messages, 'interval', hours=24) # ЧТОБЫ РАБОТАЛ scheduler ДЛЯ УВЕДОМЛЕНИЙ, ПОСТАВЬТЕ hours=1
scheduler.start()
